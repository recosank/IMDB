...
export const getTrailer = async (req,res) => {
    ...
    try {
        const t = await bannerModelDb.create({
            ...
            MImage:
            {
                data:fs.readFileSync(path.join(__dirname + '/uploads/' + req.files.MImage[0].filename)),
            },
            PImage:
            {
                data:fs.readFileSync(path.join(__dirname + '/uploads/' + req.files.PImage[0].filename)),
            }
        })
        res.send('success')
    } 
...
export const getAll = async (req,res) =>{
  console.log(req.userId)
    try {
        ...
        const wl = req.userId ? await watchlistDb.findOne({user:req.userId}) : []
        const tt = {
            pp,
            ff,
            yy,
...
export const ratingUpdate = async (req,res) => {
    ...
    try {
        ...
        if (trailerFind.ratings.length > 0 && trailerFind.ratings.some(val => { return val._id.toString() == req.userId.toString()})){
            await bannerModelDb.updateOne({'ratings._id':req.userId},{$set:{'ratings.$.rating':iid.rating}},{
                upsert: true,
                runValidators: true
            })
            ...
            console.log(req.userId)
            console.log('psh')
            await bannerModelDb.findOneAndUpdate({_id:iid._id}
            ,{
                $push:{
                    ratings:{
                        _id:req.userId,
